<p align="center">
  <img src="/github_banner_odjo.svg" alt="ODJO Banner" />
</p>

# ODJO Mahugnon Justin 👋
### Full Stack & Mobile Developer

🌱 Développeur Full Stack & Mobile spécialisé dans la création d’applications web, mobiles et desktop avec Spring Boot, Vue.js, Flutter, React Native et Visual Basic.

## 💻 Ce que je développe
- Applications web modernes et APIs performantes
- Applications mobiles multiplateformes
- Outils et scripts d’automatisation

🚀 **Compétences :**
- Front-end : Vue.js, React Native, Flutter
- Back-end : Spring Boot, Laravel, APIs REST
- Base de données : MySQL, PostgreSQL
- Outils : Git, GitHub, CI/CD

## 🚀 Stack Technique

<p align="left">
  <img src="https://skillicons.dev/icons?i=vue,spring,flutter,react,laravel,postgres,mysql,git,docker" />
</p>


📫 **Contact :**
- Email : justinodjo07@gmail.com
- LinkedIn : [JUSTINODJO](https://www.linkedin.com/in/justin-odjo-71b860179/)
- Portfolio : [ODJO](https://www.odjo.tech)

✨ Passionné par la création de solutions utiles, performantes et centrées utilisateur.

## Mes projets

### 🏡 Hweon — Plateforme de gestion immobilière
- **Tech** : Vue.js, Spring Boot, Bootstrap  
- **Rôle** : Développeur full-stack  
- **Description** : Application complète de gestion immobilière développée comme projet personnel.
- **Fonctionnalités** : authentification sécurisée, gestion des annonces, planification des visites, gestion des transactions, notifications pour les gestionnaires, propriétaires et locataires, interface responsive. 
- **Lien code** : [GitHub](https://github.com/Justinodjo/hweon/)  
- **Démo Live** : [Voir l’application](https://dev.mainsdorplus.com)

  <img width="1680" height="954" alt="Capture d’écran 2026-01-27 à 06 34 33" src="https://github.com/user-attachments/assets/fca27a6d-a39b-494c-93a7-bac5c11f2428" /><br>


  <img width="1680" height="954" alt="Capture d’écran 2026-01-27 à 06 35 36" src="https://github.com/user-attachments/assets/dc780002-7610-463b-8bda-363debfa588e" /><br>


  <img width="1680" height="954" alt="Capture d’écran 2026-01-27 à 06 37 08" src="https://github.com/user-attachments/assets/36691b09-2bfc-42b7-a268-e9a552b94f3b" />




### Application Mobile Flutter
- **Tech** : Flutter, freezed  
- **Rôle** : Développeur mobile  
- **Description** : Application mobile de gestion et lecture de podcasts. 
- **Lien code** : [GitHub](https://github.com/Justinodjo/radionova)

<img width="1024" height="1024" alt="ux_design" src="https://github.com/user-attachments/assets/f4432545-060b-4fb1-9abf-7432d380f08d" />


### Application Web Vue.js
- **Tech** : Vue.js, Spring Boot, Bootstap, FedaPay
- **Rôle** : Développeur full-stack  
- **Description** : Application web de réservation pour l’institut Mains d’Or Plus.
- **Lien code** : [GitHub](https://github.com/Justinodjo/mainsdorplus)

<img width="1680" height="929" alt="Capture d’écran 2026-01-27 à 07 38 12" src="https://github.com/user-attachments/assets/c7ea843d-95b6-4ba0-a2d5-3b28f50baf51" />

<img width="1680" height="887" alt="Capture d’écran 2026-01-27 à 07 37 51" src="https://github.com/user-attachments/assets/6fc4b586-d328-4be5-aa09-3493d40a57f1" />

<img width="1680" height="929" alt="Capture d’écran 2026-01-27 à 07 38 50" src="https://github.com/user-attachments/assets/4aa36e0b-6e10-4f55-88e2-629d56b84a74" />


## 🔥 Actuellement

- Développement d’applications SaaS
- Architecture backend avec Spring Boot
- Déploiement cloud et CI/CD
- Applications mobiles Flutter


## 📊 GitHub Stats

![Justin GitHub stats](https://github-readme-stats.vercel.app/api?username=Justinodjo&show_icons=true&theme=tokyonight)
